...
export default function StudySessions() {
  ...
  const sessionsByDate = data.studySessions.reduce((acc, session) => {
    ...
    <div className="pb-20 lg:pb-0">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">Track and manage your study time</p>
        </div>
          ...
          <DialogContent className="glass sm:max-w-md">
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <Select 
                  value={newSession.subjectId?.toString() || ""} 
                  onValueChange={(value) => setNewSession({ ...newSession, subjectId: value ? parseInt(value) : null })}
                >
                  <SelectTrigger>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div>
          <TimerCard subjects={data.subjects} onComplete={handleTimerComplete} />
        </div>
      </div>
        ...
        <Card className="glass">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-center max-w-md">
              You haven't recorded any study sessions yet. Use the timer or add a session manually.
            </p>
            <Button 
              className="mt-4"
        ...
        <div>
          ...
          {sortedDates.map((date) => (
            <div key={date} className="mb-6">
              <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">{date}</h4>
              <Card className="glass">
                    ...
                    {sessionsByDate[date].map((session) => (
                      <li key={session.id} className="p-4 flex items-center justify-between">
                        ...
                        <div className="flex items-center">
                          ...
                          <span className="font-medium">{session.duration.toFixed(1)} hours</span>
                        </div>
                      </li>
                    ))}
                  </ul>
